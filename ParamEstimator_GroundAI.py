...
# as seen by a ground-based detector in a vertical configuration..
#
...
from Analysis_Scripts import antennaFuns_ground


###################################################
# import the noise curve data
noise_curve_data = np.loadtxt('Analysis_Scripts/Instruments_Sensitivity/NoiseCurve_Groundkm.dat').T
...
def fGW_tau_PN(tau, nu, m, PN_order):
   """
   Returns the GW frequency (in [Hz]) of the signal
...
def get_htilde(
   ...
   Detector inputs:
      detector_t0 -- reference time at which to define the location of 
                           the detector [seconds after vernal equinox]
      detector_RA0 -- right ascension of the detector at t0 [rad]
      detector_DEC0 -- declination of the detector at t0 [rad]
   ...
   timeDelay, Fp, Fc = antennaFuns_ground.timeDelay_antennaFuns(
      source_tc - tau_vec,
      source_RA,
      source_DEC,
      source_psi,
...
class GW_event:
   def __init__(
      ...
      Detector inputs:
         ...
         detector_t0 -- reference time at which to define the location of 
                        the detector [seconds after vernal equinox]
         detector_RA0 -- right ascension of the detector at t0 [rad]. For a 
                         detector network, enter as np.array
         detector_DEC0 -- declination of the detector at t0 [rad]. For a 
                          detector network, enter as np.array
      Parameters for computation:
         PN_order_phase -- order of PN expansion of phase
         PN_order_amplitude -- order of PN expansion of waveform
      ...
      if np.isnan(self.max_time_spacing):
         self.max_time_spacing = 1e-2*24*3600
   ...
   def get_SNR_FisherMatrix(self):
      ...
      if np.array(self.detector_RA0).ndim == 0:
         ...
         for i in range(len(self.detector_RA0)):
            run_params_list.append((
               self.source_Mc,
               self.source_q,
               self.source_iota,
      ...
      Nruns = len(run_params_list)
      # run the Fisher matrix calculation
      starttime = time.time() 
      if self.verbose:
         print("Starting calculation of FisherMat.")
         print(str(int(Nruns)), " frequency-domain chunks will be computed")
      temp_results = []
      for i in range(Nruns):
         ...
         if self.verbose:
            print(
               ...
               " of ",
               str(Nruns),
               " frequency-domain chunks computed; runtime: ",
               "{:.1f}".format(time.time() - starttime),
               " sec"
               )
      # collect results
      self.SNR2 = 0.
