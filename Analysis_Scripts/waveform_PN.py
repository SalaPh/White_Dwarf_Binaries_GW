...
def theta(t, nu, m, tc=0.):
   ...
   """
   return nu*phconst.c**3/(5.*(phconst.G*const.SolarMass)*m)*(tc-t)
...
def find_theta_fGW(nu, m, PN_order, fGW):
   ...
   Inputs:
      ...
      fGW -- GW frequency [Hz]
   """
   x0 = (np.pi*(phconst.G*const.SolarMass)*m*fGW/phconst.c**3)**(2./3.)
   try:
...
def tau(theta, nu, m):
   ...
   Inputs:
      ...
      m -- total mass of the binary [Msol]
   """
   return (5.*(phconst.G*const.SolarMass)*m)/(nu*phconst.c**3)*theta
...
def psi_PN(theta, nu, mu, m, PN_order, theta0, Omega0):
   ...
   M = M_ADM(m, E)
   Omega = phconst.c**3/((phconst.G*const.SolarMass)*m)*x**1.5
   return phi - 2.*(phconst.G*const.SolarMass)/phconst.c**3*M*Omega*np.log(Omega/Omega0)
...
def h_plus_PN(nu, mu, Delta, x, psi, iota, dL, PN_order):
   ...
   return 2.*(phconst.G*const.SolarMass)*mu*x/phconst.c**2/dL*const.Mpc_per_m*out 
...
def get_hp_hc(
   ...
   Omega0 = phconst.c**3/((phconst.G*const.SolarMass)*m)*x_vec[0]**1.5
