...
def orbit_earth_sun(t):
   ...
   input:
      t -- time in [seconds] measured from the vernal equinox
   output:
      3-vector of the position of the Earth in [m] in a heliocentric
...
def period_satellite_earth(R):
   ...
   around Earth
   input:
      R -- radius of orbit around the center of the Earth in [m]
   output:
      period of satellite orbit in [seconds]
   '''
...
def orbit_satellite_earth(t, R, t0, RA0, DEC0, period=np.nan):
   '''
   position in [m] of a satellite orbiting Earth
   in Earth-centric coordinates
   input:
      t -- time in [seconds]
      ...
      period -- period of satellite orbit. If no input, the period is
                ...
                Earth
   output:
      3-vector of the position of the satellite in [m] in an
      Earth-centric equatorial coordinate system
   '''
   if np.isnan(period):
...
def baseline_direction(t, R, t0, RA0, DEC0, period=np.nan):
   '''
   direction of the baseline for a constellation of two
   ...
   input:
      ...
      period -- period of satellite orbit. If no input, the period is
                ...
                Earth
   output:
      3-vector of the direction of the baseline in equatorial coordinates
   '''
   if np.isnan(period):
      period = period_satellite_earth(R)
   RotMat2 = np.array([
...
def timeDelay_antennaFuns(
   ...
   '''
   Time delay and antenna functions for a constellation of two trailing satellites
   in orbit around Earth.
   Note: the rotation of the detector around Earth is deactivated because it
   generates multiple critical points for the stationary phase approximation.
   This movement is very well known and can be computed out.
   input:
   ...
   # compute time delay in frame at the position of the Earth
   # at clock_tD and the antenna funs
   if time_vec.ndim == 0:
      r = (
         orbit_earth_sun(time_vec)
         - orbit_earth_sun(clock_tD)
#Deactivated because it gives raise to multiple critical point, and can be 
#taken into account by knowing the orbit of the satellite
      ...
      out_timeDelay = np.dot(nhat, r)/c_m_s
      baselinehat = baseline_direction(
         time_vec,#np.ones(time_vec.shape), #
         detector_R, 
         detector_t0,   
         detector_RA0, 
         detector_DEC0, 
...
#taken into account by knowing the orbit of the satellite
      ...
      out_timeDelay = np.dot(nhat, r.T)/c_m_s
      baselinehat = baseline_direction(
         time_vec, # np.ones(time_vec.shape), # 
         detector_R, 
         detector_t0, 
      ...
      print("Abort code")
      sys.exit()
   return out_timeDelay, out_Fp, out_Fc
